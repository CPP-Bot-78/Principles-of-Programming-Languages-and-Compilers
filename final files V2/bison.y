...
must_atributes_values: STRING  { // αν είναι string ελέγχουμε μεσω της strcmp(0 αν είναι ίδια) αν έχει μια εκ των δύο παρακάτω τιμών, αλλιώς error
                            ...
                            printf("\nError line %d: Allowed android:layout_width and android:layout_height values are \"wrap_content\", \"match_parent\" or an positive integer number.\n", new_line);
                            exit(1);
                        }
...
radio_group: LEFTSYMBOL RADIOGROUP must_atributes
            ...
            LEFTSYMBOL ENDSYMBOL RADIOGROUP {
                ...
                if(!checkbutton_on_radio_list){ // αν δεν βρεθεί τυπώνουμε error
                    fprintf(stderr, "\nError line %d: The android:checkedButton value should match with a RadioButton Id value\n", new_line);
                    exit(1); //έξοδος από το πρόγραμμά με σφάλμα
                }
                else if (num_of_radio_buttons != wanted_radio_buttons){ // Ερ 3. Aν ο αριθμός των radio buttons δεν ειναι αυτός του έξτρα στοιχείου τυπώνουμε error
                    fprintf(stderr, "\nError line %d: The number of radio buttons is not the same as the android:radio_button_number value\n", new_line);
                    exit(1); //έξοδος από το πρόγραμμά με σφάλμα
                }
                else { // καθαρίζουμε το checkbutton, τον πίνακα με τα id radio buttons, το πληθος αυτων των ids σε περιπτωση που ακολουθει άλλο radio group και το num_of_radio_buttons
                    checkbutton=strdup("");
                    memset(radio_button_ids_memory, '\0', sizeof(num_of_radio_button_ids));
                    num_of_radio_button_ids=0;
                    num_of_radio_buttons=0;
                }
            } RIGHTSYMBOL;

radio_button: LEFTSYMBOL RADIOBUTTON {num_of_radio_buttons++;} must_atributes
...
padding_feature: /*empty*/ | PADDING EQUAL QUOTES POSINT { if (atoi(yytext) <=0) { // ερ 2c, αν padding <=0 -> error & exit
        printf("\nError line %d: Allowed android:padding values are positive integer numbers.\n", new_line);
        exit(1);
            }
        }  QUOTES;
...
progress_feature: /*empty*/ | PROGRESS EQUAL QUOTES POSINT {
    if (atoi(yytext)<0 || atoi(yytext)>max_value) { 
        fprintf(stderr, "Error line %d: This is not an allowed value. The value entered should be between 0 and the max value defined\n"), new_line;
        exit(1); //έξοδος από το πρόγραμμά με σφάλμα
    }
} QUOTES;
//_____________________________________________________________________________
